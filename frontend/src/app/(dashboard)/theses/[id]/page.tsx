	...
	if (error || !thesis) {
		return (
			<EmptyState
				...
				description={
					error ?? "Đề tài không tồn tại hoặc bạn không có quyền xem."
				}
				action={
	...
	return (
		<div>
				...
				<div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
					<div className="flex items-center gap-3 flex-wrap">
						...
						{(thesis.start_date || thesis.end_date) && (
							<span className="text-[12px] text-tertiary">
								Kỳ nghiên cứu:{" "}
								{formatPeriod(thesis.start_date, thesis.end_date)}
							</span>
						)}
					</div>
					...
					<div className="flex items-center gap-2 flex-wrap">
						{canSubmit && (
							<Button
								...
								onClick={() =>
									void run(
										() => thesesApi.submit(thesis.id),
										"Đã gửi đề tài cho giảng viên duyệt.",
									)
								}>
								Gửi duyệt
							</Button>
							...
							<Button
								...
								onClick={async () => {
									...
									if (
										!ok &&
										confirm(
											"Vẫn còn mốc chưa hoàn thành. Bạn có chắc muốn kết thúc đề tài?",
										)
									) {
										void run(
                ...
                trạng thái cuối. */}
						...
						<div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-4 border-t border-[var(--border-secondary)]">
							{[
								...
								[
									"Hoàn thành",
									thesis.completed_at ? formatDate(thesis.completed_at) : "—",
								],
							].map(([label, value]) => (
						...
						<Card className="p-5">
							...
							<div className="flex flex-col gap-2">
								<Button
									...
									onClick={() =>
										router.push(`/milestones?thesis=${thesis.id}`)
									}>
									Quản lý mốc tiến độ
								</Button>
								<Button
						...
						<Button
							...
							onClick={async () => {
								if (!revisionNote.trim())
									return toast.error("Vui lòng nhập nội dung cần sửa.");
								const ok = await run(
									() =>
			...
			<ConfirmDialog
				...
				message={
					<>
						Đề tài <strong className="text-primary">{thesis.title}</strong> sẽ
						bị ẩn khỏi hệ thống cùng toàn bộ mốc tiến độ và tài liệu của nó. Chỉ
						xóa được đề tài đang ở trạng thái Nháp.
					</>
				}
			/>
		</div>
...
function MemberPanel({
	...
	const [transferring, setTransferring] = React.useState<ThesisMember | null>(
		null,
	);
	...
	const handleAdd = async () => {
		...
		try {
			...
			setEmailError(
				isApiError(err) ? err.message : "Không thêm được thành viên.",
			);
		} finally {
			setAdding(false);
		}
	...
	const handleTransferLeader = async () => {
		if (!transferring?.user_id) return;

		setBusy(true);
		try {
			const updated = await thesesApi.transferLeader(
				thesis.id,
				transferring.user_id,
			);
			onChanged(updated);
			toast.success(`Đã chuyển vai trò Leader cho ${transferring.full_name}.`);
			setTransferring(null);
		} catch (err) {
			toast.error(
				isApiError(err) ? err.message : "Không chuyển được vai trò Leader.",
			);
		} finally {
			setBusy(false);
		}
	...
	return (
		<>
								...
								{canManage && m.role !== "OWNER" && m.user_id !== null && (
									<div className="flex items-center gap-1.5 flex-shrink-0">
										<button
											...
											title="Chuyển vai trò Leader cho sinh viên này"
											className="text-[11.5px] text-accent hover:underline px-2 py-0.5 rounded hover:bg-[var(--accent-subtle)] transition-colors cursor-pointer">
											Đặt làm Leader
										</button>
										<button
			...
			<ConfirmDialog
				...
				message={
					<>
						<strong className="text-primary">{removing?.full_name}</strong> sẽ
						không còn truy cập được đề tài này. Mốc tiến độ, tài liệu và bình
						luận họ đã tạo vẫn được giữ nguyên.
					</>
				}
			/>
			...
			<ConfirmDialog
				...
				title="Chuyển vai trò Leader đề tài?"
				confirmLabel="Xác nhận chuyển Leader"
				loading={busy}
				onConfirm={() => void handleTransferLeader()}
				message={
					<>
						Bạn có chắc chắn muốn chuyển vai trò <strong>Leader</strong> cho{" "}
						<strong className="text-primary">{transferring?.full_name}</strong>{" "}
						không? Sinh viên này sẽ chịu trách nhiệm chính quản lý đề tài.
					</>
				}
			/>
		</>
